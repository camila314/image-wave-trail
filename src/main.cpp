...
class $modify(PlayerObject) {
	...
	bool init(int player, int ship, GJBaseGameLayer* gameLayer, cocos2d::CCLayer* layer, bool playLayer) {
		...
		m_fields->m_texture = CCTextureCache::get()->addImage(string::pathToString(path).c_str(), true);

		if (!m_fields->m_texture)
			m_fields->m_texture = CCTextureCache::get()->addImage("sliderBar.png", true);

		m_fields->m_newStreak = new StreakClip;
		m_fields->m_newStreak->init(m_waveTrail);
		m_fields->m_newStreak->autorelease();

		if (m_waveTrail && PlayLayer::get())
	...
	CCSprite* newStreak(CCPoint const& pos, float rotation) {
		auto spr = CCSprite::createWithTexture(m_fields->m_texture);
